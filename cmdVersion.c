...
static ModModule *AddModule(const char *name);

static ModModule *FindModule(const char *name, ModModule **prev);

static ModName *AddName(const char *name, ModName **start, ModModule *module);

static ModName *FindName(const char *name, ModName *start, ModName **prev);
...
int	cmdModuleVersion(ClientData client_data, Tcl_Interp *interp, int argc,
			 CONST84 char *argv[])
{
    char	*version, *module;
    ModModule	*modptr;
    ...
    *version++ = '\0';
	...
	if (strcmp(argv[i], _default) &&
	    FindName(argv[i], modptr->name, &tmp)) {
	...
	if ((ModName *)NULL == (nameptr = AddName(argv[i],
						  &modptr->name,
						  modptr))) {
	    if (OK != ErrorLogger(ERR_INTERAL, LOC, NULL)) {
		break;
...
int cmdModuleAlias(ClientData client_data, Tcl_Interp *interp, int argc,
		   CONST84 char *argv[])
{
    ModName	*tmp, *ptr;
    const char	*version, *module;
    ModName	*trg_alias;
    ModModule	*modptr;
    
    /* not sure if these are the right things to initialize these two as: */
#ifdef PACKAGE
    module = PACKAGE;
#endif /* PACKAGE */
    ...
    /**
     **  Does any alias record exist with this name?
     **  If one does exist, then we should be finished ...
     **/
    if (ptr = FindName(argv[1], aliaslist, &tmp)) {
	if (!ptr->ptr || !ptr->ptr->name || !trg_alias &&
	    (!ptr->ptr->module || !ptr->ptr->module->module)) {
	    ErrorLogger(ERR_INTERAL, LOC, NULL);
	    return (TCL_ERROR); /** -------- EXIT (FAILURE) -------> **/
	}
	
	if (trg_alias && !strcmp(ptr->ptr->name, argv[2]) || !trg_alias &&
	    !strcmp(ptr->ptr->name, version) &&
	 ...
	 **/
	if ((ModName *)NULL == (ptr = AddName(argv[1],
					      &aliaslist, NULL))) {
	    ErrorLogger(ERR_INTERAL, LOC, NULL);
	    return (TCL_ERROR); /** -------- EXIT (FAILURE) -------> **/
	}
...
int	VersionLookup(	char *name, char **module, char **version)
{
	...
	*module = buffer;
    ...
    if ((ModName **)NULL == (history = (ModName **)module_malloc((size_t)((unsigned long)histsize * sizeof(ModName *))))) {
	...
	if((ModName *) NULL != (vptr = FindName( *version, mptr->name, &vtmp))){
	    ...
	    *version = vptr->version->name;
		...
		if (!(history = (ModName **)module_realloc(history,
							   ((unsigned long)histsize * sizeof(ModName *))))) {
		    ErrorLogger(ERR_ALLOC, LOC, NULL);
		    return (0);		/** -------- EXIT (FAILURE) -------> **/
		}
	    }
...
static	ModModule *FindModule(const char *name,
			      ModModule	**prev)
{
