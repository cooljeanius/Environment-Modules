...
dnl# Test for AUTOOPTS, & define AUTOGEN, AUTOOPTS_CFLAGS, AUTOGEN_LDFLAGS
...
[AS_HELP_STRING([--disable-opts-test],[Do not try to run a test AutoOpts program])
  ...
  if test "x${AUTOOPTS_CONFIG}" = "xno"; then
    ...
    if test "x${enable_opts_test}" != "xno" ; then
      ...
      dnl# check things used by conftest before actually doing it:
      AC_CHECK_HEADERS_ONCE([autoopts/options.h stdio.h])
      AC_CHECK_FUNCS_ONCE([system strcpy sscanf printf])
      dnl#
...
*** If autoopts-config was correct, then you should remove the old ver.\n\\
*** of autoopts. You may be able to fix the error by modifying your\n\\
...
*** the correct copy of autoopts-config, and remove your config.cache\n\\
...
main(int argc, char ** argv)
{
    ...
    if (ct != 2) {
        printf("bad vers. string: -->>%s<<-- != -->>${aocfg_currev}<<--\n",
               optionVersion());
        return 1;
    }
    ...
    if (OPTIONS_VER_TO_NUM(${aocfg_current}, ${aocfg_revision})
        ...
        printf("*** autoopts header file version "OPTIONS_VERSION_STRING"\n"
               "*** does not match autoopts-config val. ${aocfg_version}\n"
  ...
  if test "x${no_autoopts}" = "x"; then
    ...
    if test "x${AUTOOPTS_CONFIG}" = "xno" ; then
      cat <<- _EOF_
	*** The autoopts-config script installed by AutoGen was not found
	*** If AutoGen was installed in PREFIX, make sure PREFIX/bin is in
	*** your path, or set the AUTOOPTS_CONFIG envvar to the
...
*** of libopts. If it is not finding libopts, you will need to set your
*** LD_LIBRARY_PATH environment variable, or edit /etc/ld.so.conf to point
*** to the installed location. Also, make sure you have run ldconfig
*** (if that is required on your system).
***
...
*** the exact error that occured. This often means AutoGen was incorrectly
